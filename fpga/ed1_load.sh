#!/bin/sh
# Device-local ED1 RX-DMA major-13 fleet volatile FPGA-load and PL driver/service rebind canary.

set -u
umask 077

PL_BINDINGS_SHA=74c4398df30d04939f80326a135336150c3d76fffb054cf71c13509a44fc5e03
SERVICES_SHA=1cd5b9f2f98c4ad0e12f7dc382d49e38960da08d04a40bf80c0db61487563726
BIND_ATTRS_SHA=14207cb15aa9789dfeb2d4fe6e9581fabd0cf8e354cc3fb720f3a119d0690aff
...
NODE=ed1
NODE_UPPER=ED1
EXPECTED_SERIAL=104473f4a313000a160009001da4d39b96
EXPECTED_BOOT_ID=a9660881-03c0-4c15-a85d-09abbcbd5a93
...
TRANSITION_SNAPSHOT_SHA=e76391fc67a6ef04850ef53284fea0695cc7f3422e31e3701b1fb877b72d84d3
...
usage() {
    cat <<'EOF'
Usage:
  rx12_v13_ed1_load.sh --plan
  rx12_v13_ed1_load.sh --preflight --input /tmp/INPUT --watchdog SCRIPT
  rx12_v13_ed1_load.sh --execute --input /tmp/INPUT --watchdog SCRIPT \
...
if [ "$MODE" = plan ]; then
    cat <<'EOF'
RX12_MAJOR13_ED1_LOAD_PLAN_V1
scope=exact RX-DMA major-13 canary volatile FPGA load; no RF; no persistent write; payload hash-bound
preconditions=ED1 identity; reviewed hashes; backup acknowledgement; recovery artifact; VERSION 0x000CDFFF; manager operating; exact five bindings; buffers 0; SPI bound; no ED/ES or UIO/IIO users; USB links intact; exact iiod
...
validate_inputs() {
    ...
    need_kv node "$INPUT" 10; [ "$NK_VALUE" = ed1 ] || refuse 10 "input_node"
    ...
    JOURNAL="$(p /tmp)/rx12_v13_ed1_load_${TOKEN}"
...
validate_recovery_ack() {
    ...
    [ "$NK_VALUE" = "/tmp/rx12_v13_ed1_load_${TOKEN}/terminal_result" ] || refuse 25 "recovery_success_marker"
...
if [ "$MODE" = preflight ]; then
    preflight
    echo "ARC3_REBIND_PREFLIGHT_PASS read_only=1 node=ed1 pre_version=$EXPECTED_PRE_VERSION ps_fclk3=$PS_FCLK3_VALUE ps_io_pll=$PS_IO_PLL_VALUE token=$TOKEN"
    exit 0
fi
...
echo "RX12_MAJOR13_ED1_VOLATILE_LOAD_PASS journal=$JOURNAL host_must_retrieve_transcript=1"
exit 0
