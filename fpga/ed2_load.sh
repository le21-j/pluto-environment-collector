#!/bin/sh
# Device-local ED2 RX-DMA major-13 fleet volatile FPGA-load and PL driver/service rebind canary.

set -u
umask 077

PL_BINDINGS_SHA=5c9b51848f27a13b50528406c6fb3cd4df06f5c678e28eb3a37892c42c629ad7
SERVICES_SHA=676de7feba017569bdcce5468042b4a1dc5ace3483135e54b4e3301a8be7a895
BIND_ATTRS_SHA=f9c18830cd75e98f5cb24454dbac79bb7b51c0b0e1739a94a6ad3bc2592f3af0
...
NODE=ed2
NODE_UPPER=ED2
EXPECTED_SERIAL=104473308d1600121c001c00c8d02cc615
EXPECTED_BOOT_ID=d87b226a-3861-4e1d-8457-fb6e9814f199
...
TRANSITION_SNAPSHOT_SHA=101c648b42d3390e62090b3fd52ebfea758f1ec5674a443ed9e60a28d33af1db
...
usage() {
    cat <<'EOF'
Usage:
  rx12_v13_ed2_load.sh --plan
  rx12_v13_ed2_load.sh --preflight --input /tmp/INPUT --watchdog SCRIPT
  rx12_v13_ed2_load.sh --execute --input /tmp/INPUT --watchdog SCRIPT \
...
if [ "$MODE" = plan ]; then
    cat <<'EOF'
RX12_MAJOR13_ED2_LOAD_PLAN_V1
scope=exact RX-DMA major-13 canary volatile FPGA load; no RF; no persistent write; payload hash-bound
preconditions=ED2 identity; reviewed hashes; backup acknowledgement; recovery artifact; VERSION 0x000CDFFF; manager operating; exact five bindings; buffers 0; SPI bound; no ED/ES or UIO/IIO users; USB links intact; exact iiod
...
validate_inputs() {
    ...
    need_kv node "$INPUT" 10; [ "$NK_VALUE" = ed2 ] || refuse 10 "input_node"
    ...
    JOURNAL="$(p /tmp)/rx12_v13_ed2_load_${TOKEN}"
...
validate_recovery_ack() {
    ...
    [ "$NK_VALUE" = "/tmp/rx12_v13_ed2_load_${TOKEN}/terminal_result" ] || refuse 25 "recovery_success_marker"
...
if [ "$MODE" = preflight ]; then
    preflight
    echo "ARC3_REBIND_PREFLIGHT_PASS read_only=1 node=ed2 pre_version=$EXPECTED_PRE_VERSION ps_fclk3=$PS_FCLK3_VALUE ps_io_pll=$PS_IO_PLL_VALUE token=$TOKEN"
    exit 0
fi
...
echo "RX12_MAJOR13_ED2_VOLATILE_LOAD_PASS journal=$JOURNAL host_must_retrieve_transcript=1"
exit 0
