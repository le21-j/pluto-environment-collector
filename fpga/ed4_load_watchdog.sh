#!/bin/sh
# Detached recovery watchdog for the ED4 RX-DMA major-13 fleet volatile-load canary.
...
if [ "$MODE" = plan ]; then
    echo "RX12_MAJOR13_ED4_LOAD_WATCHDOG_PLAN_V1 payload_sha256=$PAYLOAD_SHA disarm=atomic_exact_candidate_load_PASS timeout=/usr/sbin/device_reboot_reset rerun=never"
    exit 0
fi
