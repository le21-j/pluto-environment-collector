#!/bin/sh
# Device-local ES RX-DMA major-13 fleet volatile FPGA-load and PL driver/service rebind canary.

set -u
umask 077

PL_BINDINGS_SHA=2c7cb8b303ef62230f3d57a973416d86ccb82dd34175fac3f4763f4f43dbf5e7
SERVICES_SHA=9640e26377e10be3cb45dbd17a5c631050288c25f72d9a3dad682e8fb0ff2fa2
BIND_ATTRS_SHA=23b77112200ceb559d93a53516af82d73640fb75b4246b2b5d890ca6c37e4230
...
TRANSITION_SNAPSHOT_SHA=eedccdc8b41d3c4f9fed94d70af5d00bb2020d117b856663c073eac01af76164
...
usage() {
    cat <<'EOF'
Usage:
  rx12_v13_es_load.sh --plan
  rx12_v13_es_load.sh --preflight --input /tmp/INPUT --watchdog SCRIPT
  rx12_v13_es_load.sh --execute --input /tmp/INPUT --watchdog SCRIPT \
...
if [ "$MODE" = plan ]; then
    cat <<'EOF'
RX12_MAJOR13_ES_LOAD_PLAN_V1
scope=exact RX-DMA major-13 canary volatile FPGA load; no RF; no persistent write; payload hash-bound
preconditions=ES identity; reviewed hashes; backup acknowledgement; recovery artifact; VERSION 0x000CDFFF; manager operating; exact five bindings; buffers 0; SPI bound; no ED/ES or UIO/IIO users; USB links intact; exact iiod
...
validate_inputs() {
    ...
    need_kv node "$INPUT" 10; [ "$NK_VALUE" = es ] || refuse 10 "input_node"
...
validate_recovery_ack() {
    ...
    [ "$NK_VALUE" = "/tmp/rx12_v13_es_load_${TOKEN}/terminal_result" ] || refuse 25 "recovery_success_marker"
...
if [ "$MODE" = preflight ]; then
    preflight
    echo "ARC3_REBIND_PREFLIGHT_PASS read_only=1 node=es pre_version=$EXPECTED_PRE_VERSION ps_fclk3=$PS_FCLK3_VALUE ps_io_pll=$PS_IO_PLL_VALUE token=$TOKEN"
    exit 0
fi
...
echo "RX12_MAJOR13_ES_VOLATILE_LOAD_PASS journal=$JOURNAL host_must_retrieve_transcript=1"
exit 0
