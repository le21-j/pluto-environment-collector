#!/bin/sh
# Device-local ED3 RX-DMA major-13 canary volatile FPGA-load and PL driver/service rebind canary.

set -u
umask 077

PL_BINDINGS_SHA=06cfaf97981924a6f03b28bbe75e658c649e7c4a9728710df29baf61afe9378e
SERVICES_SHA=f1f119927973c1d09626ece7ce1105b60f9b99ed189bfc8aa73040a856ef6fb3
BIND_ATTRS_SHA=2e09c5f6ce34a737dc3bfda80f96abf7f319d0e7e6e3cb259597ec2e498ac27e
...
NODE=ed3
NODE_UPPER=ED3
EXPECTED_SERIAL=104473308d160004ebff1e000bfe2c7b36
EXPECTED_BOOT_ID=045d1c97-c054-4715-a61e-f1b773d40726
...
TRANSITION_SNAPSHOT_SHA=ae8b5c3bbcca414c98a41d7dd812b55511eccc827be67628f8533c96d48b30f7
...
usage() {
    cat <<'EOF'
Usage:
  rx12_v13_ed3_load.sh --plan
  rx12_v13_ed3_load.sh --preflight --input /tmp/INPUT --watchdog SCRIPT
  rx12_v13_ed3_load.sh --execute --input /tmp/INPUT --watchdog SCRIPT \
...
if [ "$MODE" = plan ]; then
    cat <<'EOF'
RX12_MAJOR13_ED3_LOAD_PLAN_V1
scope=exact RX-DMA major-13 canary volatile FPGA load; no RF; no persistent write; payload hash-bound
preconditions=ED3 identity; reviewed hashes; backup acknowledgement; recovery artifact; VERSION 0x000CDFFF; manager operating; exact five bindings; buffers 0; SPI bound; no ED/ES or UIO/IIO users; USB links intact; exact iiod
...
validate_inputs() {
    ...
    need_kv node "$INPUT" 10; [ "$NK_VALUE" = ed3 ] || refuse 10 "input_node"
    ...
    JOURNAL="$(p /tmp)/rx12_v13_ed3_load_${TOKEN}"
...
validate_recovery_ack() {
    ...
    [ "$NK_VALUE" = "/tmp/rx12_v13_ed3_load_${TOKEN}/terminal_result" ] || refuse 25 "recovery_success_marker"
...
if [ "$MODE" = preflight ]; then
    preflight
    echo "ARC3_REBIND_PREFLIGHT_PASS read_only=1 node=ed3 pre_version=$EXPECTED_PRE_VERSION ps_fclk3=$PS_FCLK3_VALUE ps_io_pll=$PS_IO_PLL_VALUE token=$TOKEN"
    exit 0
fi
...
echo "RX12_MAJOR13_VOLATILE_LOAD_PASS journal=$JOURNAL host_must_retrieve_transcript=1"
exit 0
